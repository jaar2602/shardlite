    ...
    <div className="space-y-4">
      ...
      {(operation.expected_versions.length > 0 || operation.outcomes.length > 0) && <div>
        ...
        <ClusterTopologyPanel
          ...
          legend={<><LegendDot tone="green">applied</LegendDot><LegendDot tone="red">rejected</LegendDot><LegendDot tone="gray">pending</LegendDot><LegendDot tone="blue">not involved</LegendDot></>}
          caption={(_node, shards) => {
            const mine = shards.filter((shard) => involved.has(shard));
            if (mine.length === 0) return "not involved";
