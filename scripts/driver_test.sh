#!/usr/bin/env bash
#
# Start a meshdb gateway on BOTH transports (HTTP and JSON/TCP) and exercise each driver against
...
if command -v python3 >/dev/null 2>&1; then
  ok "python" "$(MESHDB_PORT=$P MESHDB_TCP_PORT=$TP python3 drivers/python/driver_check.py 2>&1)"
else echo "  python  skipped (absent)"; fi

if command -v node >/dev/null 2>&1; then
  ok "node  " "$(MESHDB_PORT=$P MESHDB_TCP_PORT=$TP node drivers/javascript/driver_check.mjs 2>&1)"
...
if command -v go >/dev/null 2>&1; then
  OUT=$(cd drivers/go && MESHDB_PORT=$P MESHDB_TCP_PORT=$TP go run ./example 2>&1)
