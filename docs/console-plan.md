# meshdb console — Phase 3 plan (for scope confirmation)
...
meshdb clusters, the way a database client (DBeaver, TablePlus) manages many database
connections. Two jobs:

1. **Database management** — connect to a meshdb cluster, browse schema, run SQL, run atomic
...
It is deliberately **not** served from the meshdb binary. That decision is already made and its
...
So Phase 3 adds **no meshdb core changes**. The one thing the DB does not provide — a *history*
...
```
  browser ──(same origin, JSON + NDJSON)──► console backend (Rust) ──(HTTP/JSON /v1)──► meshdb cluster(s)
...
```
console/
  server/            Rust backend — its own binary + crate, NOT in the meshdb workspace
    Cargo.toml
    src/
...
The backend reaches meshdb over the **HTTP/JSON edge**, not the native bincode client, so the
console can be released and upgraded independently of the exact meshdb build it points at.
...
Small, sync, standard-library-leaning, matching meshdb's discipline — but this is a separate
crate outside the meshdb workspace, so its dependency choices don't touch the DB's footprint.
...
- **Console auth (multi-user).** A console user store — username + password hash + a console
  ...
  from the per-connection meshdb credentials.
- **Connection registry.** CRUD of connection profiles: name, meshdb address, transport,
  and the meshdb credential (a meshdb username + secret). Persisted to a file. The stored
...
- **Proxy + streaming.** The SPA never talks to meshdb directly; the backend forwards to the
  selected connection's `/v1`. Large query results **stream straight through** (NDJSON in from
  meshdb, NDJSON out to the browser) so the "handle 1 row to 1 million rows" robustness carries
...
- **Serve the SPA.** The built frontend is embedded into the backend binary so the console
  ships as **one self-contained executable** (matches meshdb's bundled-SQLite spirit).
...
- **Users** — list/create/drop meshdb users with roles (admin-gated; `/v1/users`).
...
6. Embed the built SPA into the backend binary; single-binary run; `console/README.md`; a smoke
   script that starts a meshdb gateway + the console and drives the API.
...
1. **Console login — multi-user from the start.** The console has its own user store: username +
   ...
   entirely separate from the meshdb credentials it stores per connection.
2. **Backend→meshdb transport — HTTP/JSON edge.** The console reaches every cluster over `/v1`,
   so it is decoupled from the exact meshdb build and survives cluster version skew.
...
console's own auth ladder — do not confuse it with meshdb's `Read/Write/Admin`, which still
governs what each stored meshdb credential can do on the cluster.
